...
async function optimizeImage(inputPath, outputPath, srcOutputPath, options = {}) {
  ...
  if (srcOutputPath) {
    await pipeline.toFile(srcOutputPath);
      ...
      if (['.jpg', '.jpeg', '.png', '.gif', '.tiff', '.webp'].includes(ext)) {
        ...
        try {
          ...
          for (const size of sizes) {
            ...
            const srcOutputPath = path.join(srcOutputBaseDir, `${baseName}-${size}w.webp`);
            await optimizeImage(fullPath, outputPath, srcOutputPath, { width: size, format: 'webp' });
          }

          // Generate original size WebP
          const webpPath = `${outputBasePath}.webp`;
          const srcWebpPath = path.join(srcOutputBaseDir, `${baseName}.webp`);
          await optimizeImage(fullPath, webpPath, srcWebpPath, { format: 'webp' });
          ...
          await optimizeImage(fullPath, jpegPath, srcJpegPath, { format: 'jpeg' });
